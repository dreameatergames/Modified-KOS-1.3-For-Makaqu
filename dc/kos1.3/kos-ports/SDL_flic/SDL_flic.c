...
static inline int readbuffer(FLI_Animation *flic, void *buffer, int size) {
        if (SDL_RWread(flic->rwops, buffer, 1, size) != size)
	{
                flic->error=FLI_READERROR;
		return flic->error;
	}
	return 0;
}
...
static inline int readheader(FLI_Animation *flic) {
...
static inline int readframe(FLI_Animation *flic, FLI_Frame *frame) {
...
static inline int readchunk(FLI_Animation *flic, FLI_Chunk *chunk) {
        // Read the chunk size. 
...
static inline int handlecolor(FLI_Animation *flic, FLI_Chunk *chunk) {
...
static inline int handlelc(FLI_Animation *flic, FLI_Chunk *chunk) {
...
static inline int handleblack(FLI_Animation *flic, FLI_Chunk *chunk) {
        (void)chunk;
        // Fill the surface with color 0. 
        if (SDL_FillRect(flic->surface, NULL, 0) != 0)
	{
                flic->error=FLI_SDLERROR;
		return flic->error;
	}
	return 0;
}

static inline int handlebrun(FLI_Animation *flic, FLI_Chunk *chunk) {
        int    numlines, size;
        Uint8  *p, *next;
...
static inline int handlecopy(FLI_Animation *flic, FLI_Chunk *chunk) {
        (void)chunk;
...
static inline int handlecolor256(FLI_Animation *flic, FLI_Chunk *chunk) {
        int       numpackets, index, count;
        SDL_Color color;
...
static inline int handless2(FLI_Animation *flic, FLI_Chunk *chunk) {
...
FLI_Animation *FLI_Open(SDL_RWops *rwops, int *error) {
	...
	{
		if (error != NULL) *error=FLI_SDLERROR;
		free(flic);
		return NULL;
	}
